...
parser.add_argument(
    "-q",
    "--sql",
    help="Use direct SQL queries instead of the XTools API to get article information. This will run much faster, but can only be done when running the software from Toolforge servers.",
    action="store_true",
)
parser.add_argument(
    "-o",
    "--host",
    help="Host for SQL server (default is 127.0.0.1 for SSH tunnel on localhost, else it should be enwiki.analytics.db.svc.wikimedia.cloud).",
    default=sqlHost,
)
parser.add_argument(
    "-m",
...
if args.explain:
    ...
    print("       Such is life.")
    print(
        "The specific task of this script is to populate the JSON skeletons of the previous script with actual information about the pages in question, obtained with XTools. After this script, and the other one that handles the actual page content, the renderer can generate a page."
    )
    print(
...
def closeOut(totalQueriesMade):
    ...
    aLog(
        ...
        + "s / "
        + str(round((execTime / totalQueriesMade), 3))
        + "s per query"
    )
    try:
...
aLog(
    "Running Oracle for Deletion (mass analyzer), version "
    + version
    + ", at "
            ...
            try:
                if useSql == 0:
                    ...
                    if (forReal) and (page != ""):
                        ...
                        urls = [
                            apiBase + page.replace("&", "%26").replace("?", "%3F"),
                            apiBase
                        ...
                        for incre in [0, 1]:
                            if (limitMaxQueries == True) and (
                                totalQueriesMade > maxQueriesToMake
                            ):
                    ...
                    for incre in [0, 1]:
                        ...
                        if r[0] == 0:
                            ...
                            original = datetime.strptime(
                                original[0:19] + "+0000", "%Y-%m-%dT%H:%M:%S%z"
                            )
                        ...
                        if verbose:
                            print(
                                storeIn + " retrieved (" + str(totalQueriesMade) + ")"
                            )
