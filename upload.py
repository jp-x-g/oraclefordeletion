...
parser = argparse.ArgumentParser(
    description="Oracle for Deletion, uploader (5 of 5).", epilog="Wow!"
)
...
parser.add_argument(
    ...
    "--note",
    metavar="TEXT",
    help="Comment to add to edit summary.",
    default="Updating with",
)
parser.add_argument(
    "-i",
    "--input",
    metavar="blahblah.txt",
    help="Input file to read, out of "
    + os.getcwd()
    + "/"
    + dataname
    + "/"
...
parser.add_argument(
    "-o",
    "--output",
    metavar="User:JohnDoe/OfD",
    help="Wikipedia page to post the file to. Default is "
    + pagename
    + ". Be careful with this one, because it is easy to do something stupid.",
    default=pagename,
)
parser.add_argument(
    "-u",
    "--username",
    metavar="JohnDoe@OfD_poster",
    help="Specify username to authenticate with. Default is to read from "
    + os.getcwd()
    + "/"
    + configname
    + "/"
    + logindeets
    + ".",
    default="Didn't specify one.",
)
parser.add_argument(
    "-p",
    "--password",
    metavar="hunter2",
    help="Specify password to authenticate with. Default is to read from "
    + os.getcwd()
    + "/"
    + configname
    + "/"
...
parser.add_argument(
    "-d",
    "--dryrun",
    help="Run the script without actually editing the page.",
    action="store_true",
)
parser.add_argument(
    "-v",
...
parser.add_argument(
    "-z",
    "--zero",
    help="Upload the bare input file (without profiling information, headers or footers).",
    action="store_true",
)
...
aLog(
    "Running Oracle for Deletion (uploader), version "
    + version
    + ", at "
...
if forReal == 1:
    edit = s.post(
        ...
        data={
            ...
            "text": textToSend,
            "summary": args.note
            + " ([[WP:OFD|Oracle for Deletion]], version "
            + version
            + ")",
            "format": "json",
        },
