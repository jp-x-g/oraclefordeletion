...
if args.explain:
	...
	print("       Such is life.")
	print("The specific task of this script is to set up JSON skeletons of AfD log pages, without much information about the pages in question. After this script, the .JSONs will be populated by the next one, which uses XTools to get page statistics, and the third one, which uses the en.wikipedia API to parse actual page content. Finally, the renderer uses all of this information to generate a page.")
	quit()
...
aLog("Running script for " + userRunning + ". Processing " + str(numberOfDays) + " days: " + today.strftime("%Y %B %d") + " back to " + (today - timedelta(days=(numberOfDays - 1))).strftime("%Y %B %d") + ".") 
...
for incr in range(0,numberOfDays):
	try:
		...
		theWikitextUrl = "https://en.wikipedia.org/w/api.php?action=query&prop=revisions&titles=Wikipedia%3AArticles_for_deletion%2FLog%2F" + dayText + "&rvslots=*&rvprop=content&formatversion=2&format=json"
		# initialize afdDay with the date it's being scraped at, as well as some other stuff.
		afdDay = {"scrapedate":today.isoformat(), "afddate":dayDate, "afddatetext":dayText, "count":0}
		if verbose:
